				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-8 mb-3">
							<div class="form-group">
								<label for="post_title" class="form-label">Judul ekskul</label>
								<input id="post_title" name="post_title" class="form-control " type="text"
									placeholder="Judul ekskul..." value="" required>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<label for="post_thumbnail" class="form-label">Foto ekskul</label>
					...
					<div class="row g-2">
						<div class="col mb-0 form-group">
							<label for="emailExLarge" class="form-label">Deskripsi ekskul</label>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-6 mb-3">
							<div class="form-group">
								<label for="epost_title" class="form-label">Judul ekskul</label>
						...
						<div class="col-md-4">
							<div class="form-group">
								<label for="epost_thumbnail" class="form-label">Foto ekskul</label>
						...
						<div class="col-md-2">
							<img src="" alt="Thumbnail ekskul" id="foto" height="100" class="w-100">
						</div>
					</div>
					<div class="row g-2">
						<div class="col mb-0 form-group">
...
<script>
	$(document).ready(function () {
		...
		$('#ekskul').on('click', '.imgekskul', function () {
		...
		function tampildata() {
			$('#ekskul').DataTable({
				...
				"paging": true,
				"ajax": {
					url: "<?php echo base_url() ?>ekskul/getdata",
					type: 'GET'
				},
			});
		}
		...
		function uploadImage(image) {
			...
			$.ajax({
				url: "<?= base_url('ekskul/upload_image') ?>",
				cache: false,
				contentType: false,
				processData: false,
				data: data,
		...
		function deleteImage(src) {
			$.ajax({
				...
				type: "POST",
				url: "<?= base_url('ekskul/delete_image') ?>",
				cache: false,
		...
		$.validator.setDefaults({
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>ekskul/simpan_ekskul",
					data: formData,
					processData: false,
					contentType: false,
					cache: false,
		...
		$('#editform').submit(function (e) {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url() ?>ekskul/simpanedit",
		...
		$('#ekskul').on('click', '.bhapus', function () {
			...
			swal.fire({
				...
				if (result.value) {
					$.ajax({
						url: '<?= base_url() ?>ekskul/hapus_ekskul',
						type: 'POST',
						dataType: 'json',
						data: {
							id: id
