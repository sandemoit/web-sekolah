					...
					<div class="row">
						<div class="col-md-8 mb-3">
							<div class="form-group">
								<label for="post_title" class="form-label">Nama industri</label>
								<input id="post_title" name="post_title" class="form-control " type="text"
									placeholder="Nama industri..." value="" required>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<label for="post_thumbnail" class="form-label">Foto industri</label>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-6 mb-3">
							<div class="form-group">
								<label for="epost_title" class="form-label">Judul industri</label>
								<input id="epost_title" name="epost_title" class="form-control " type="text"
									placeholder="Judul industri..." value="" required>
						...
						<div class="col-md-4">
							<div class="form-group">
								<label for="epost_thumbnail" class="form-label">Foto industri</label>
						...
						<div class="col-md-2">
							<img src="" alt="Thumbnail industri" id="foto" height="100" class="w-100">
						</div>
					</div>
...
<script>
	$(document).ready(function () {
		...
		$('#industri').on('click', '.imgindustri', function () {
		...
		function tampildata() {
			$('#industri').DataTable({
				...
				"paging": true,
				"ajax": {
					url: "<?php echo base_url() ?>industri/getdata",
					type: 'GET'
				},
			});
		}
		...
		function uploadImage(image) {
			...
			$.ajax({
				url: "<?= base_url('industri/upload_image') ?>",
				cache: false,
				contentType: false,
				processData: false,
				data: data,
		...
		function deleteImage(src) {
			$.ajax({
				...
				type: "POST",
				url: "<?= base_url('industri/delete_image') ?>",
				cache: false,
		...
		$.validator.setDefaults({
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>industri/simpan_industri",
					data: formData,
					processData: false,
					contentType: false,
					cache: false,
		...
		$('#editform').submit(function (e) {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url() ?>industri/simpanedit",
		...
		// showedit data
		$('#industri').on('click', '.bedit', function () {
			var id = $(this).attr('data');
			$.ajax({
				type: "GET",
				url: "<?= base_url() ?>industri/showedit",
				dataType: "JSON",
				data: {
					id: id
				},
				success: function (data) {
					$.each(data, function () {
						...
						$('[id="foto"]').attr("src",
							"<?= base_url() ?>uploads/industri/" + data
							.foto);
		...
		$('#industri').on('click', '.bhapus', function () {
			var id = $(this).attr('data');
			swal.fire({
				title: 'Yakin Menghapus data industri ini?',
				...
				if (result.value) {
					$.ajax({
						url: '<?= base_url() ?>industri/hapus_industri',
						type: 'POST',
						dataType: 'json',
						data: {
							id: id
