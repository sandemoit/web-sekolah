	...
	<div class="card mb-4">
		...
		<div class="card-body">
			...
			<div class="table-responsive text-nowrap">
				<table id="keutamaan" class="table">
					<thead>
						<tr>
							<th>No</th>
							<th>Foto</th>
							<th>Keutamaan/Layanan</th>
							<th>Action</th>
						</tr>
					</thead>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-8 mb-3">
							<div class="form-group">
								<label for="post_title" class="form-label">Nama Keutamaan/Layanan</label>
								<input id="post_title" name="post_title" class="form-control " type="text"
									placeholder="Nama keutamaan..." value="" required>
							</div>
						</div>
						<div class="col-md-4">
							<div class="form-group">
								<label for="post_thumbnail" class="form-label">Foto keutamaan</label>
					...
					<div class="row g-2">
						<div class="col mb-0 form-group">
							<label for="emailExLarge" class="form-label">Deskripsi keutamaan</label>
				...
				<div class="modal-body">
					...
					<div class="row">
						<div class="col-md-6 mb-3">
							<div class="form-group">
								<label for="epost_title" class="form-label">Judul keutamaan</label>
								<input id="epost_title" name="epost_title" class="form-control " type="text"
									placeholder="Judul keutamaan..." value="" required>
						...
						<div class="col-md-4">
							<div class="form-group">
								<label for="epost_thumbnail" class="form-label">Foto keutamaan</label>
						...
						<div class="col-md-2">
							<img src="" alt="Thumbnail keutamaan" id="foto" height="100" class="w-100">
						</div>
					</div>
					<div class="row g-2">
						<div class="col mb-0 form-group">
...
<script>
	$(document).ready(function () {
		...
		$('#keutamaan').on('click', '.imgkeutamaan', function () {
		...
		function tampildata() {
			$('#keutamaan').DataTable({
				...
				"paging": true,
				"ajax": {
					url: "<?php echo base_url() ?>keutamaan/getdata",
					type: 'GET'
				},
			});
		}
		...
		function uploadImage(image) {
			...
			$.ajax({
				url: "<?= base_url('keutamaan/upload_image') ?>",
				cache: false,
				contentType: false,
				processData: false,
				data: data,
		...
		function deleteImage(src) {
			$.ajax({
				...
				type: "POST",
				url: "<?= base_url('keutamaan/delete_image') ?>",
				cache: false,
		...
		$.validator.setDefaults({
			submitHandler: function (form) {
				...
				$.ajax({
					type: "POST",
					url: "<?= base_url() ?>keutamaan/simpan_keutamaan",
					data: formData,
					processData: false,
					contentType: false,
					cache: false,
					...
					success: function (response) {
						if (response == "success") {
							...
							$('#keutamaan').DataTable().ajax.reload();
							$('[name="post_title"]').val("");
		...
		$('#editform').submit(function (e) {
			...
			$.ajax({
				type: "POST",
				url: "<?= base_url() ?>keutamaan/simpanedit",
		...
		// showedit data
		$('#keutamaan').on('click', '.bedit', function () {
			var id = $(this).attr('data');
			$.ajax({
				type: "GET",
				url: "<?= base_url() ?>keutamaan/showedit",
				dataType: "JSON",
				data: {
					id: id
				},
				success: function (data) {
					$.each(data, function () {
						...
						$('[id="foto"]').attr("src",
							"<?= base_url() ?>uploads/keutamaan/" + data
							.foto);
		...
		$('#keutamaan').on('click', '.bhapus', function () {
			var id = $(this).attr('data');
			swal.fire({
				title: 'Yakin Menghapus data keutamaan ini?',
				...
				if (result.value) {
					$.ajax({
						url: '<?= base_url() ?>keutamaan/hapus_keutamaan',
						type: 'POST',
						dataType: 'json',
						data: {
							id: id
