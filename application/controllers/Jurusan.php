...
class Jurusan extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->path = FCPATH . "uploads/jurusan/";
		$this->path2 = FCPATH . "uploads/jurusan/isi/";
	...
	public function index()
	{
		...
		$this->load->view('admin/page/jurusan',$data);
	...
	public function getdata()
	{
		...
		$query = $this->db->query(" SELECT	jurusan.*  FROM jurusan  ORDER BY jurusan.id DESC");
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = '
			  ...
			  <img height="100" class="imgjurusan" data="ahsudah" data-bs-toggle="modal" data-bs-target="#myModal" src="' . base_url() . 'uploads/jurusan/' . $lists->foto . '"  alt="' . $lists->foto . '"/>
	...
	public function showedit()
	{
		...
		$user = $this->db->query("select * from jurusan where id = '$koser'");
		if ($user->num_rows() > 0) {
	...
	public function simpanedit()
	{
		...
		$slug = $this->Settings_model->create_slug($judul);
		$cariold = $this->db->get_where('jurusan', array('id' => $id))->row();
		...
		if (!empty($_FILES['epost_thumbnail']['name'])) {
			...
			$data = $this->db->update('jurusan', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function hapus_jurusan()
	{
		...
		$jurusan = $this->db->query("select * from jurusan where id ='$id' ")->row();

		if ($jurusan) {
			$this->deleteContentImage($jurusan->desc);
			$img = $jurusan->foto;
			$image = $this->path . $img;
	...
	public function upload_image()
	{
		if (isset($_FILES["image"]["name"])) {
			$path = FCPATH . 'uploads/jurusan/isi/';
			...
			if (!$this->upload->do_upload('image')) {
				...
				$this->load->library('image_lib', $config);
				$this->image_lib->resize();
				echo base_url('uploads/jurusan/isi/') . $data['file_name'];
			}
		}
	}
