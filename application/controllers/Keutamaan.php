...
class Keutamaan extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->path = FCPATH . "uploads/keutamaan/";
		$this->path2 = FCPATH . "uploads/keutamaan/isi/";
	...
	public function index()
	{
		...
		$this->load->view('admin/page/keutamaan',$data);
	...
	public function getdata()
	{
		...
		$query = $this->db->query(" SELECT	keutamaan.*  FROM keutamaan  ORDER BY keutamaan.id DESC");
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = '
			  ...
			  <img height="100" class="imgkeutamaan" data="ahsudah" data-bs-toggle="modal" data-bs-target="#myModal" src="' . base_url() . 'uploads/keutamaan/' . $lists->foto . '"  alt="' . $lists->foto . '"/>
	...
	function simpan_keutamaan()
	{
		...
		if (!empty($_FILES['post_thumbnail']['name'])) {
			...
			$data = $this->db->insert('keutamaan', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function showedit()
	{
		...
		$user = $this->db->query("select * from keutamaan where id = '$koser'");
		if ($user->num_rows() > 0) {
	...
	public function simpanedit()
	{
		...
		$slug = $this->Settings_model->create_slug($judul);
		$cariold = $this->db->get_where('keutamaan', array('id' => $id))->row();
		...
		if (!empty($_FILES['epost_thumbnail']['name'])) {
			...
			$data = $this->db->update('keutamaan', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function hapus_keutamaan()
	{

		$id = $this->input->post('id');
		$keutamaan = $this->db->query("select * from keutamaan where id ='$id' ")->row();

		if ($keutamaan) {
			$this->deleteContentImage($keutamaan->desc);
			$img = $keutamaan->foto;
			$image = $this->path . $img;
			...
			$data = $this->db->query("DELETE FROM keutamaan WHERE id = '$id' ");
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function upload_image()
	{
		if (isset($_FILES["image"]["name"])) {
			$path = FCPATH . 'uploads/keutamaan/isi/';
			...
			if (!$this->upload->do_upload('image')) {
				...
				$this->load->library('image_lib', $config);
				$this->image_lib->resize();
				echo base_url('uploads/keutamaan/isi/') . $data['file_name'];
			}
		}
	}
