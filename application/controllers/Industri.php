...
class Industri extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->path = FCPATH . "uploads/industri/";
		$this->path2 = FCPATH . "uploads/industri/isi/";
	...
	public function index()
	{
		...
		$this->load->view('admin/page/industri',$data);
	...
	public function getdata()
	{
		...
		$query = $this->db->query(" SELECT	industri.*  FROM industri  ORDER BY industri.id DESC");
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = '
			  ...
			  <img height="100" class="imgindustri" data="ahsudah" data-bs-toggle="modal" data-bs-target="#myModal" src="' . base_url() . 'uploads/industri/' . $lists->foto . '"  alt="' . $lists->foto . '"/>
	...
	function simpan_industri()
	{
		...
		if (!empty($_FILES['post_thumbnail']['name'])) {
			...
			$data = $this->db->insert('industri', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function showedit()
	{
		...
		$user = $this->db->query("select * from industri where id = '$koser'");
		if ($user->num_rows() > 0) {
	...
	public function simpanedit()
	{
		...
		$slug = $this->Settings_model->create_slug($judul);
		$cariold = $this->db->get_where('industri', array('id' => $id))->row();
		...
		if (!empty($_FILES['epost_thumbnail']['name'])) {
			...
			$data = $this->db->update('industri', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function hapus_industri()
	{
		...
		$industri = $this->db->query("select * from industri where id ='$id' ")->row();

		if ($industri) {
			$this->deleteContentImage($industri->desc);
			$img = $industri->foto;
			$image = $this->path . $img;
			...
			$data = $this->db->query("DELETE FROM industri WHERE id = '$id' ");
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function upload_image()
	{
		if (isset($_FILES["image"]["name"])) {
			$path = FCPATH . 'uploads/industri/isi/';
			...
			if (!$this->upload->do_upload('image')) {
				...
				$this->load->library('image_lib', $config);
				$this->image_lib->resize();
				echo base_url('uploads/industri/isi/') . $data['file_name'];
			}
		}
	}
