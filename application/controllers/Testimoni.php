...
class Testimoni extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->path = FCPATH . "uploads/testimoni/";
		$this->path2 = FCPATH . "uploads/testimoni/isi/";
	...
	public function getdata()
	{
		...
		$query = $this->db->query(" SELECT	testimoni.*  FROM testimoni  ORDER BY testimoni.id DESC");
		...
		foreach ($query->result() as $key => $lists) {
			...
			$data[$key][]  = '
			  ...
			  <img height="100" class="imgtestimoni" data="ahsudah" data-bs-toggle="modal" data-bs-target="#myModal" src="' . base_url() . 'uploads/testimoni/' . $lists->foto . '"  alt="' . $lists->foto . '"/>
	...
	function simpan_testimoni()
	{
		...
		if (!empty($_FILES['post_thumbnail']['name'])) {
			...
			$data = $this->db->insert('testimoni', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function showedit()
	{
		...
		$user = $this->db->query("select * from testimoni where id = '$koser'");
		if ($user->num_rows() > 0) {
	...
	public function simpanedit()
	{
		...
		$slug = $this->Settings_model->create_slug($judul);
		$cariold = $this->db->get_where('testimoni', array('id' => $id))->row();
		...
		if (!empty($_FILES['epost_thumbnail']['name'])) {
			...
			$data = $this->db->update('testimoni', $datasimpan);
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function hapus_testimoni()
	{
		...
		$testimoni = $this->db->query("select * from testimoni where id ='$id' ")->row();

		if ($testimoni) {
			$this->deleteContentImage($testimoni->desc);
			$img = $testimoni->foto;
			$image = $this->path . $img;
			...
			$data = $this->db->query("DELETE FROM testimoni WHERE id = '$id' ");
			if ($data) {
				echo 'success';
			} else {
				echo 'error';
	...
	public function upload_image()
	{
		if (isset($_FILES["image"]["name"])) {
			$path = FCPATH . 'uploads/testimoni/isi/';
			...
			if (!$this->upload->do_upload('image')) {
				...
				$this->load->library('image_lib', $config);
				$this->image_lib->resize();
				echo base_url('uploads/testimoni/isi/') . $data['file_name'];
			}
		}
	}
